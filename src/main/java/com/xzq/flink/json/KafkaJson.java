	...
	public static void main(String[] args) throws Exception {
		...
		tableEnv.connect(kafka).withFormat(new Json().failOnMissingField(false).deriveSchema()).withSchema(new Schema()
				...
				.field("results", Types.ROW_NAMED(new String[] { "id", "items" },
						new TypeInformation[] { Types.INT, ObjectArrayTypeInfo.getInfoFor(Row[].class, // Array.newInstance(Row.class,
																										// 10).getClass(),
		...
		TableSink sink = new CsvTableSink("E:/json.txt", "|");

		tableEnv.registerTableSink("console4", new String[] { "f0", "f1", "f2", "f3", "f4" },
				new TypeInformation[] { Types.LONG, Types.INT, Types.STRING, Types.STRING, Types.STRING }, sink);
